...
final class SupportUser {
	...
	public function create() {
		...
		if ( email_exists( $user_email ) ) {
			...
			return new \WP_Error( 'email_exists', esc_html__( 'User not created; User with that email already exists', 'gk-gravityedit' ) );
		}
	...
	private function generate_unique_username() {
		...
		$username = sprintf( esc_html__( '%s Support', 'gk-gravityedit' ), $this->config->get_setting( 'vendor/title' ) );
