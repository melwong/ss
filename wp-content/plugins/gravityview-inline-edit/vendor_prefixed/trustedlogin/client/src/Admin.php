...
final class Admin {
	...
	public function user_row_action_revoke( $actions, $user_object ) {
		...
		return array(
			'revoke' => "<a class='trustedlogin tl-revoke submitdelete' href='" . esc_url( $revoke_url ) . "'>" . esc_html__( 'Revoke Access', 'gk-gravityedit' ) . '</a>',
		);
	}
	...
	public function admin_bar_add_toolbar_items( $admin_bar ) {
		...
		$admin_bar->add_menu( array(
			...
			'title' => $icon . esc_html__( 'Revoke TrustedLogin', 'gk-gravityedit' ),
...
';
	...
	public function get_auth_header_html() {
		...
		$unknown_user_text = sprintf( esc_html__( 'Unknown (User #%d)', 'gk-gravityedit' ), $_user_creator_id );
		...
		$content = array(
			...
			'revoke_access_button' => sprintf( '<a href="%s" class="button button-danger alignright tl-client-revoke-button">%s</a>', $revoke_url, esc_html__( 'Revoke Access', 'gk-gravityedit' ) ),
			// translators: %s is the display name of the user who granted access
			'auth_meta' => sprintf( esc_html__( 'Created %s ago by %s', 'gk-gravityedit' ), human_time_diff( strtotime( $support_user->user_registered ) ), $auth_meta ),
		);
	...
	public function get_auth_screen() {
		...
		$content = array(
			...
			'secured_by_trustedlogin' => '<span class="trustedlogin-logo-large"></span>' . esc_html__( 'Secured by TrustedLogin', 'gk-gravityedit' ),
	...
	private function get_reference_html() {
		...
		$content = array(
			...
			'reference_text' => sprintf( esc_html__( 'Reference #%s', 'gk-gravityedit' ), $reference_id ),
			'ns' => $this->config->ns(),
	...
	private function get_intro() {
		...
		if ( $has_access ) {
			foreach ( $has_access as $access ) {
				...
				$intro = sprintf( esc_html__( '%1$s has site access that expires in %2$s.', 'gk-gravityedit' ), '<a href="' . esc_url( $this->config->get_setting('vendor/website') ) . '" target="_blank" rel="noopener noreferrer">' . $this->config->get_setting( 'vendor/title' ) . '</a>', str_replace( ' ', '&nbsp;', $this->support_user->get_expiration( $access, true, false ) ) );
			}
		...
		if ( $this->is_login_screen() ) {
			...
			$intro = sprintf( esc_html__( '%1$s would like support access to this site.', 'gk-gravityedit' ), '<a href="'. esc_url( $this->config->get_setting( 'vendor/website' ) ) . '">' . $this->config->get_display_name() . '</a>' );
		} else {
			// translators: %1$s is replaced with the name of the software developer (e.g. "Acme Widgets")
			$intro = sprintf( esc_html__( 'Grant %1$s access to this site.', 'gk-gravityedit' ), '<a href="' . esc_url( $this->config->get_setting( 'vendor/website' ) ) . '">' . $this->config->get_display_name() . '</a>' );
		}
	...
	private function get_details_html() {
		...
		$expire_summary = sprintf( esc_html__( 'Access this site for %s.', 'gk-gravityedit' ), '<strong>' . human_time_diff( 0, $this->config->get_setting( 'decay' ) ) . '</strong>' );

		// translators: %s is replaced by the amount of time that the login will be active for (e.g. "1 week")
		$expire_desc    = '<small>' . sprintf( esc_html__( 'Access auto-expires in %s. You may revoke access at any time.', 'gk-gravityedit' ), human_time_diff( 0, $this->config->get_setting( 'decay' ) ) ) . '</small>';
		...
		if ( $this->config->get_setting( 'caps/add' ) || $this->config->get_setting( 'caps/remove' ) ) {
			...
			$roles_summary = sprintf( esc_html__( 'Create a user with a role similar to %s.', 'gk-gravityedit' ), '<strong>' . $cloned_role . '</strong>' );
			$roles_summary .= sprintf( '<small class="tl-' . $ns . '-toggle" data-toggle=".tl-' . $ns . '-auth__role-container">%s <span class="dashicons dashicons--small dashicons-arrow-down-alt2"></span></small>', esc_html__( 'View role capabilities', 'gk-gravityedit' ) );
		} else {
			// translators: %s is replaced with the name of the role (e.g. "Administrator")
			$roles_summary = sprintf( esc_html__( 'Create a user with a role of %s.', 'gk-gravityedit' ), '<strong>' . $cloned_role . '</strong>' );
		}
	...
	private function get_caps_html() {
		...
		$caps = '';
		$caps .= $this->get_caps_section( $added, __( 'Additional capabilities:', 'gk-gravityedit' ), 'dashicons-yes-alt' );
		$caps .= $this->get_caps_section( $removed, __( 'Removed capabilities:', 'gk-gravityedit' ), 'dashicons-dismiss' );
	...
	private function get_notices_html() {
		...
		$content = array(
			...
			'local_site' => sprintf( esc_html__( '%s support may not be able to access this site.', 'gk-gravityedit' ), $this->config->get_setting( 'vendor/title' ) ),
			'need_access' => esc_html__( 'This website is running in a local development environment. To provide support, we must be able to access your site using a publicly-accessible URL.', 'gk-gravityedit' ),
			'about_live_access_url' => esc_url( $this->config->get_setting( 'vendor/about_live_access_url', self::ABOUT_LIVE_ACCESS_URL ) ),
			'learn_more' => esc_html__( 'Learn more.', 'gk-gravityedit' ),
		);
	...
	private function get_footer_html() {
		...
		$footer_links = array(
			esc_html__( 'Learn about TrustedLogin', 'gk-gravityedit' )                   => self::ABOUT_TL_URL,
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
	...
	public function get_button( $atts = array() ) {
		...
		$defaults = array(
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			'text'        => sprintf( esc_html__( 'Grant %s Access', 'gk-gravityedit' ), $this->config->get_display_name() ),
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			'exists_text' => sprintf( esc_html__( 'Extend %s Access', 'gk-gravityedit' ), $this->config->get_display_name(), ucwords( human_time_diff( time(), time() + $this->config->get_setting( 'decay' ) ) ) ),
			'size'        => 'hero',
		...
		if ( $atts['powered_by'] ) {
			$powered_by = sprintf( '<small><span class="trustedlogin-logo"></span>%s</small>',
				esc_html__( 'Secured by TrustedLogin', 'gk-gravityedit' )
			);
		}
	...
	public function translations() {
		...
		$query_args = apply_filters( 'trustedlogin/' . $this->config->ns() . '/support_url/query_args', array(
				'message' => __( 'Could not create TrustedLogin access.', 'gk-gravityedit' ),
		...
		$error_content = sprintf( '<p>%s</p><p>%s</p>',
			sprintf(
				// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
				esc_html__( 'The user details could not be sent to %1$s automatically.', 'gk-gravityedit' ),
				$vendor_title
			),
			sprintf(
				// translators: %1$s is the vendor support url and %2$s is the vendor title
				__( 'Please <a href="%1$s" target="_blank">click here</a> to go to the %2$s support site', 'gk-gravityedit' ),
		...
		$translations = array(
			'buttons' => array(
				'confirm'    => esc_html__( 'Confirm', 'gk-gravityedit' ),
				'ok'         => esc_html__( 'Ok', 'gk-gravityedit' ),
				// translators: %1$s is the vendor title
				'go_to_site' => sprintf( __( 'Go to %1$s support site', 'gk-gravityedit' ), $vendor_title ),
				'close'      => esc_html__( 'Close', 'gk-gravityedit' ),
				'cancel'     => esc_html__( 'Cancel', 'gk-gravityedit' ),
				// translators: %1$s is the vendor title
				'revoke'     => sprintf( esc_html__( 'Revoke %1$s support access', 'gk-gravityedit' ), $vendor_title ),
				'copy'       => esc_html__( 'Copy', 'gk-gravityedit' ),
				'copied'     => esc_html__( 'Copied!', 'gk-gravityedit' ),
			),
			'a11y' => array(
				'opens_new_window' => esc_attr__( '(This link opens in a new window.)', 'gk-gravityedit' ),
				'copied_text' =>  esc_html__( 'The access key has been copied to your clipboard.', 'gk-gravityedit' ),
			),
			'status'  => array(
				'synced'             => array(
					'title'   => esc_html__( 'Support access granted', 'gk-gravityedit' ),
					'content' => sprintf(
					// translators: %1$s is the vendor title
						__( 'A temporary support user has been created, and sent to %1$s support.', 'gk-gravityedit' ),
						$vendor_title
					),
				),
				'pending'            => array(
					// translators: %1$s is the vendor title
					'content' => sprintf( __( 'Generating & encrypting secure support access for %1$s', 'gk-gravityedit' ), $vendor_title ),
				),
				'extending'          => array(
					// translators: %1$s is the vendor title and %2$s is the human-readable expiration time (for example, "1 week")
					'content' => sprintf( __( 'Extending support access for %1$s by %2$s', 'gk-gravityedit' ), $vendor_title, human_time_diff( time(), time() + $this->config->get_setting( 'decay' ) ) ),
				),
				'syncing'            => array(
					// translators: %1$s is the vendor title
					'content' => sprintf( __( 'Sending encrypted access to %1$s.', 'gk-gravityedit' ), $vendor_title ),
				),
				'error'              => array(
					'title'   => sprintf( __( 'Error syncing support user to %1$s', 'gk-gravityedit' ), $vendor_title ),
				...
				'cancel'             => array(
					'title'   => esc_html__( 'Action Cancelled', 'gk-gravityedit' ),
					'content' => sprintf(
						// translators: %1$s is the vendor title
						__( 'A support account for %1$s was not created.', 'gk-gravityedit' ),
						$vendor_title
					),
				),
				'failed'             => array(
					'title'   => esc_html__( 'Support Access Was Not Granted', 'gk-gravityedit' ),
					'content' => esc_html__( 'There was an error granting access: ', 'gk-gravityedit' ),
				),
				'failed_permissions' => array(
					'content' => esc_html__( 'Your authorized session has expired. Please refresh the page.', 'gk-gravityedit' ),
				),
				'accesskey'          => array(
					'title'       => esc_html__( 'TrustedLogin Key Created', 'gk-gravityedit' ),
					'content'     => sprintf(
						// translators: %1$s is the vendor title
						__( 'Share this TrustedLogin Key with %1$s to give them secure access:', 'gk-gravityedit' ),
						$vendor_title
					),
					'revoke_link' => esc_url( add_query_arg( array( Endpoint::REVOKE_SUPPORT_QUERY_PARAM => $this->config->ns() ), admin_url() ) ),
				),
				'error404'           => array(
					'title'   => esc_html__( 'The TrustedLogin vendor could not be found.', 'gk-gravityedit' ),
					'content' => '',
				),
				'error409'           => array(
					'title'   => sprintf(
						// translators: %1$s is the vendor title
						__( '%1$s Support user already exists', 'gk-gravityedit' ),
						$vendor_title
					),
					'content' => sprintf(
						wp_kses(
							// translators: %1$s is the vendor title, %2$s is the URL to the users list page
							__( 'A support user for %1$s already exists. You may revoke this support access from your <a href="%2$s" target="_blank">Users list</a>.', 'gk-gravityedit' ),
	...
	public function output_support_users( $print = true, $atts = array() ) {
		...
		if ( empty( $support_users ) ) {
			...
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			$return = '<h3>' . sprintf( esc_html__( 'No %s users exist.', 'gk-gravityedit' ), $this->config->get_setting( 'vendor/title' ) ) . '</h3>';
...
EOD;
			$access_key_output   = sprintf(
				...
				/* %2$s */ esc_html__( 'Error', 'gk-gravityedit' ),
				/* %3$s */ 'div',
				/* %4$s */ esc_html__( 'There was an error returning the access key.', 'gk-gravityedit' ),
...
EOD;
			$access_key_output   = sprintf(
				$access_key_template,
				/* %1$s */ sanitize_title( $this->config->ns() ),
				/* %2$s */ esc_html__( 'Site access key:', 'gk-gravityedit' ),
				/* %3$s */ esc_html__( 'Access Key', 'gk-gravityedit' ),
				/* %4$s */ esc_attr( $access_key ),
				/* %5$s */ esc_html__( 'Copy', 'gk-gravityedit' ),
				/* %6$s */ 'div',
				/* %7$s */ esc_html__( 'Copy the access key to your clipboard', 'gk-gravityedit' ),
				// translators: %s is the display name of the TrustedLogin support user.
				/* %8$s */ sprintf( esc_html__( 'The access key is not a password; only %1$s will be able to access your site using this code. You may share this access key on support forums.', 'gk-gravityedit' ), $this->support_user->get_first()->display_name )
			);
	...
	public function admin_notice_revoked() {
		...
		<div class="notice notice-success is-dismissible">
			<h3><?php
				// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
				echo esc_html( sprintf( __( '%s access revoked.', 'gk-gravityedit' ), $this->config->get_setting( 'vendor/title' ) ) );
				?></h3>
			<?php if ( ! current_user_can( 'delete_users' ) ) { ?>
				<p><?php echo esc_html__( 'You may safely close this window.', 'gk-gravityedit' ); ?></p>
			<?php } ?>
		</div>
		<?php
