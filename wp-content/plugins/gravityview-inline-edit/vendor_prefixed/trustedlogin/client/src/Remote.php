...
final class Remote {
	...
	static public function check_response_code( $api_response ) {
		...
		switch ( $response_code ) {
			...
			case 400:
			case 423:
				return new \WP_Error( 'unable_to_verify', esc_html__( 'Unable to verify Pause Mode.', 'gk-gravityedit' ), $api_response );

			case 401:
				return new \WP_Error( 'unauthenticated', esc_html__( 'Authentication failed.', 'gk-gravityedit' ), $api_response );

			case 402:
				return new \WP_Error( 'account_error', esc_html__( 'TrustedLogin account issue.', 'gk-gravityedit' ), $api_response );
	...
	public function handle_response( $api_response, $required_keys = array() ) {
		...
		if ( empty( $response_json ) ) {
			return new \WP_Error( 'invalid_response', esc_html__( 'Invalid response.', 'gk-gravityedit' ), $response_body );
		}
