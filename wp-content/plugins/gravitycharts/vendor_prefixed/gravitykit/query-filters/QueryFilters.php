...
class QueryFilters {
	...
	private function convert_filters_to_gf_conditions( array $filters ) {
		$convert = function ( &$filter ) use ( &$convert ) {
			if ( ! empty( $filter['mode'] ) && isset( $filter['conditions'] ) ) {
				...
				$field = \GFAPI::get_field( $this->form['id'], $key );
				...
				if ( $field && in_array( $field->type, [ 'total' ] ) && in_array( $filter->operator, [ \GF_Query_Condition::LT, \GF_Query_Condition::GT ] ) ) {
	...
	private function get_translations() {
		...
		$translations = apply_filters( 'gk/query-filters/translations', [
			'internet_explorer_notice' => esc_html__( 'Internet Explorer is not supported. Please upgrade to another browser.', 'gk-gravitycharts' ),
			'fields_not_available'     => esc_html__( 'Form fields are not available. Please try refreshing the page.', 'gk-gravitycharts' ),
			...
			'remove_field'             => esc_html__( 'Remove Field', 'gk-gravitycharts' ),
			'available_choices'        => esc_html__( 'Return to Field Choices', 'gk-gravitycharts' ),
			'available_choices_label'  => esc_html__( 'Return to the list of choices defined by the field.', 'gk-gravitycharts' ),
			'custom_is_operator_input' => esc_html__( 'Custom Choice', 'gk-gravitycharts' ),
			'untitled'                 => esc_html__( 'Untitled', 'gk-gravitycharts' ),
			'field_not_available'      => esc_html__( 'Form field ID #%d is no longer available. Please remove this condition.', 'gk-gravitycharts' ),
		] );
	...
	public function enqueue_scripts( array $meta = array() ) {
		...
		$ver    = rgar( $meta, 'ver', filemtime( plugin_dir_path( __FILE__ ) . $script ) );
		$src    = rgar( $meta, 'src', plugins_url( $script, __FILE__ ) );
	...
	public static function enqueue_styles( array $meta = array() ) {
		...
		$handle = rgar( $meta, 'handle', self::ASSETS_HANDLE );
		$ver    = rgar( $meta, 'ver', filemtime( plugin_dir_path( __FILE__ ) . $style ) );
		$src    = rgar( $meta, 'src', plugins_url( $style, __FILE__ ) );
